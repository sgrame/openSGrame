 ...
 * SG_File_Mime
 * 
 * Mime mappings based on file extention.
 * Based on the mapping system of Drupal
 * @link http://api.drupal.org/api/drupal/includes%21file.mimetypes.inc/7
 *
 * @category SG
 * @package  File
 * @author   Peter Decuyper <sgrame@gmail.com>
...
class SG_File_Mime
{
    ...
    /**
     * File extention mapping 
     */
    static $extentions = array(
        'ez'       => 0,
        'atom'     => 1,
        'atomcat'  => 2,
        'atomsrv'  => 3,
     ...
     *     mime type 
     */
    static function mimeFromFilename($filename)
    {
        $match = array();

        // get the extention
        if (!preg_match('/\.([A-Za-z0-9]+)$/', $filename, $match)) {
            return self::mimeFromExtention(null);
        }
        
        return self::mimeFromExtention($match[1]);
    }
    
    /**
     * Get the mime type from the file extention
     * 
     ...
     *     mime type
     */
    static function mimeFromExtention($extention)
    {
        $mime      = 'application/octet-stream';
        $extention = strtolower($extention);
        
        if (!isset(self::$extentions[$extention])) {
            return $mime;
        }
        
        if (!isset(self::$mimeTypes[self::$extentions[$extention]])) {
            return $mime;
        }
        
        return self::$mimeTypes[self::$extentions[$extention]];
    }
}
